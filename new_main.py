...
def Data_exp(df, to_drop):
    ...
    df['mvar16'] = (df.mvar16 + df.mvar17 + df.mvar18)/3
    df['mvar19'] = (df.mvar19 + df.mvar20 )/2
    
    df['avg_tenure_on_oldest_credits'] = (df.mvar26 +df.mvar27 +df.mvar28 +df.mvar29)/4
    
##############
    
    df['Max_tenure_on_loans'] = (df.mvar30 + df.mvar31)/2
    df.drop(['mvar31','mvar30'], axis = 1)
    
    df['w_sum_cred_missed'] = ((df.mvar34)/2 + (df.mvar35)/0.5)/2.5
    df.drop(['mvar34','mvar35'], axis = 1)
    
    df['avg_utilization'] = (df.mvar40 + df.mvar41)/2
    df.drop(['mvar40','mvar41'], axis = 1)
    
    df['mvar1'] = df.mvar1/df.mvar42 
...
if with_pca ==1 :
    ...
    classifier.fit(XX_train, train_y, early_stopping_rounds = 10, 
                   eval_set=[(XX_test, val_y)], verbose=True)
    ...
    classifier.fit(imp_train_X, train_y, early_stopping_rounds=10, 
                   eval_set=[(imp_val_X, val_y)], verbose=True)
...
if submit == 1:
    ...
    if with_pca ==1 :
        ...
        classifier1 = xgb.sklearn.XGBClassifier(base_score=0.5, colsample_bylevel=1, colsample_bytree=1,
               gamma=0, learning_rate=0.05, max_delta_step=0, max_depth=3,
               min_child_weight=1, missing=None, n_estimators=121, nthread=-1,
               objective='binary:logistic', reg_alpha=0, reg_lambda=1,
